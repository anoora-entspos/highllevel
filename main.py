...
api_headers={
            ...
            "Authorization": f"Bearer {runpod_auth}"
        }
...
async def create_animation(request:AnimationRequest):
    try:
        ...
        print(f"Request error during SD3 image generation: {e}")
        return {"error": "Failed to interact with SD3 service"}, 500
    except Exception as e:
        print(f"Unexpected error during SD3 image generation: {e}")
        return {"error": "Internal server error"}, 500

    try:
        # Step 4: Call Live Portrait API with the uploaded image
        ...
        print(f"Request error during Live Portrait: {e}")
        return {"error": "Failed to interact with Live Portrait service"}, 500
    except Exception as e:
        print(f"Unexpected error during Live Portrait: {e}")
        return {"error": "Internal server error"}, 500

    try:
        # Step 7: Generate speech using Eleven Labs API
        ...
        print(f"Request error during speech generation: {e}")
        return {"error": "Failed to interact with Eleven Labs API"}, 500
    except Exception as e:
        print(f"Unexpected error during speech generation: {e}")
        return {"error": "Internal server error"}, 500

    try:
        # Step 9: Request video retalking model to sync image and audio
        # Step 9.1: fetch length of audio
        ...
        # Step 10: Check video retalking model status (poll every 15 seconds)
        ...
        # Step 11: Convert base64 video to MP4 and upload to Firebase and sd3 image
        ...
        # Upload video and image to Firebase, add document to Firestore
        upload_video_to_firebase(temp_video_path, BytesIO(base64.b64decode(base64_image)), prompt, userid)
        ...
        print(f"Request error during video retalking: {e}")
        return {"error": "Failed to interact with video retalking service"}, 500
    except Exception as e:
        print(f"Unexpected error during video retalking: {e}")
        return {"error": "Internal server error"}, 500
